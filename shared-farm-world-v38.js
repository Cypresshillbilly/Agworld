...
async function sync(){const c=client(),u=window.AGWorldBackend?.getUser?.();if(!c||!u)return false;const{data,error}=await c.from('ag_farm_events').select('*').order('created_at',{ascending:true});if(error){console.warn('Shared farm sync failed',error);return false;}(data||[]).forEach(e=>{last[e.id]=1;apply(e)});return true}
...
function connect(){const c=client();if(!c||sub)return;sub=c.channel('agworld-farm-events').on('postgres_changes',{event:'INSERT',schema:'public',table:'ag_farm_events'},p=>{last[p.new.id]=1;apply(p.new)}).subscribe()}
function find(farm){const fs=window.__AG_WORLD_FARMS||[];return fs.find(x=>farmId(x)===String(farm))||farm}
window.AGWorldSharedFarms={sync,event,addOurDrone:f=>event(find(f),'add_our_drone'),addCompetitor:(f,name)=>event(find(f),'add_competitor',{name}),removeCompetitor:f=>event(find(f),'remove_competitor'),updateDetails:(f,patch)=>event(find(f),'update_details',patch),connect};
window.addEventListener('agworld:farms-reset',()=>{sync().catch(error=>console.warn('Shared farm replay failed',error));});
