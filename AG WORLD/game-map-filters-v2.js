...
(() => {
  ...
  async function loadData(){
    // Demo territory test mode has one authoritative source: app-gis-loader.
    // Never fetch the legacy API, data/farms.json or browser storage here,
    // otherwise their old farms recreate the red default Google pins.
    if (window.AG_WORLD_DEMO_MODE) {
      clear();
      return;
    }
    let farms=[];
  ...
  function addMarker(f){
    if(window.AG_WORLD_DEMO_MODE || f.demo)return;
