...
(() => {
  ...
  function showGate(){
    ...
    // The login gate owns the interactive period. Do not load or initialise the
    // Supabase SDK while the player is typing: on this application the SDK parse
    // can contend with keyboard rendering and produce visible input lag.
    // Authentication resources are warmed only once the player submits.
    const startAgworldAuthWarmup=()=>ensureAgworldSupabase();
    ...
    gate.querySelector('form').addEventListener('submit',async e=>{
      ...
      if(!master){
        ...
        try{
          // The player has finished typing. Start SDK/client work only now so
          // the entire credential-entry period remains input-first and free of
          // background authentication parsing.
          const db=await startAgworldAuthWarmup();
