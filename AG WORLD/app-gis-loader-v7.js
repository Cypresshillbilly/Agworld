...
function normaliseSpatialFeatures(geojson, level) {
  ...
  return features.map((feature, index) => {
    ...
    const name = p.MAP_TITLE || p.MUNICNAME || p.NameCode || p.S12_NAME || p.SGADMIN || p.SGTOWN || p.TOWN || p.NAME || p.name || `${level} ${index + 1}`;
    ...
    return {
      ...
      control: 0,
      owner: 'Unassigned',
      source: level === 'municipality' ? 'Municipal Demarcation Board / DLRRD' : 'Surveyor-General / DLRRD',
      boundary,
      center,
      properties: p
    };
...
async function loadSpatialLayersInBackground() {
  console.info('AG World GIS loader v7 started');
  ...
  $('mapStatus').textContent = 'GIS loader v7 started · Municipalities: connecting… · Towns: connecting…';
  ...
  $('mapStatus').textContent = errors.length
    ? `GIS finished · ${municipalities.length} Municipalities · ${towns.length} Towns · failed: ${errors.join(' + ')} · ${errorDetails.join(' | ')}`
...
function addTerritory(territory) {
  ...
  const styles = {
    ...
    province: { strokeColor: '#8fb339', fillColor: '#8fb339', strokeWeight: 2.5, fillOpacity: .03 },
  ...
  const polygon = new google.maps.Polygon({
    ...
    clickable: true,
    zIndex: level === 'town' ? 4 : level === 'municipality' ? 3 : level === 'province' ? 2 : 1,
    map
  });
...
function refreshMapVisibility() {
  if (!map) return;
  const zoom = map.getZoom();
  // Remove any legacy placeholder province overlays if they exist.
  territories.forEach(territory => {
    if (territory._polygon) territory._polygon.setMap(null);
    if (territory._marker) territory._marker.setMap(null);
  });
  ...
  // Do not render locally stored province rectangles. These records are only
  // used for metadata, ownership and hierarchy until/where GIS geometry is used.
