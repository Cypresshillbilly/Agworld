...
assert(auth.includes("if(gameLoader) gameLoader.classList.add('is-active')"),'Auth must activate Loading V0');
assert(auth.includes('await window.__AGWORLD_BOOT_GAME__()'),'Auth must call canonical deferred V1 boot');
assert(auth.includes('await playerReady'),'Auth must wait for canonical Player V1 readiness');
...
assert(loginPanel.includes('boot/v1/performance.js?v=boot-performance-v1-20260912'),'Performance module is not wired into Login V1 boundary');
...
for(const [name,source] of [
  ['login/v1/auth.js',auth],
  ['login/v1/login-panel.js',loginPanel],
  ['boot/v1/performance.js',performanceModule],
  ['app-gis-loader-v24.js',read('app-gis-loader-v24.js')]
]){
