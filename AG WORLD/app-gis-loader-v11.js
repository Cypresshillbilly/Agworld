...
const TERRITORY_GAME_DEFAULTS = {
  country: { baseValue: 1000000, income: 100000, development: 1 },
  province: { baseValue: 250000, income: 25000, development: 1 },
  municipality: { baseValue: 50000, income: 5000, development: 1 },
  town: { baseValue: 10000, income: 1000, development: 1 },
  farm: { baseValue: 2500, income: 250, development: 1 }
};

function territoryGameKey(territory) {
  return `agworld-territory-state-${territory.level || 'province'}-${territory.id}`;
}

function territoryGameDefaults(territory) {
  const level = territory.level || 'province';
  const defaults = TERRITORY_GAME_DEFAULTS[level] || TERRITORY_GAME_DEFAULTS.province;
  return {
    owner: territory.owner || 'Unassigned',
    status: territory.status || 'Available',
    control: Number.isFinite(Number(territory.control)) ? Number(territory.control) : 0,
    value: Number(territory.value || defaults.baseValue),
    income: Number(territory.income || defaults.income),
    development: Number(territory.development || defaults.development),
    influence: Number(territory.influence || 0),
    acquiredAt: territory.acquiredAt || null
  };
}

function applyTerritoryGameState(territory) {
  if (!territory) return territory;
  const defaults = territoryGameDefaults(territory);
  let stored = {};
  try { stored = JSON.parse(localStorage.getItem(territoryGameKey(territory)) || '{}'); } catch (_) {}
  territory.game = { ...defaults, ...stored };
  territory.owner = territory.game.owner;
  territory.status = territory.game.status;
  territory.control = territory.game.control;
  territory.value = territory.game.value;
  territory.income = territory.game.income;
  territory.development = territory.game.development;
  territory.influence = territory.game.influence;
  return territory;
}

function persistTerritoryGameState(territory) {
  if (!territory?.game) return;
  localStorage.setItem(territoryGameKey(territory), JSON.stringify(territory.game));
}

function initialiseGameTerritories() {
  [...countries, ...territories, ...municipalities, ...towns].forEach(applyTerritoryGameState);
}
...
function territoryGameSummary(territory) {
  const descendants = territoryDescendants(territory);
  const all = [territory, ...descendants];
  const territoryFarms = farms.filter(f => {
    const ids = new Set(all.map(x => x.id));
    return ids.has(f.townId) || ids.has(f.municipalityId) || ids.has(f.territoryId);
  });
  return {
    descendants: descendants.length,
    farms: territoryFarms,
    owned: all.filter(t => t.game?.owner && t.game.owner !== 'Unassigned').length,
    available: all.filter(t => !t.game?.owner || t.game.owner === 'Unassigned').length,
    income: all.reduce((sum, t) => sum + (Number(t.game?.income) || 0), 0),
    value: all.reduce((sum, t) => sum + (Number(t.game?.value) || 0), 0)
  };
}
...
function normaliseSpatialFeatures(geojson, level) {
  ...
  }).filter(x => x.boundary.length >= 3).map(applyTerritoryGameState);
}
...
async function loadSpatialLayersInBackground() {
  console.info('AG World GIS loader v11 started');
...
function selectTerritory(territory, zoom = true) {
  applyTerritoryGameState(territory);
  ...
  $('farmMeta').textContent = `${levelLabel} TERRITORY · ${game.status} · ${game.owner || 'Unassigned'}`;
  ...
  $('farmScore').textContent = `${game.control}%`;
  $('farmLivestock').textContent = territoryFarms.reduce((n, f) => n + (Number(f.livestock) || 0), 0) || '—';
  $('farmHarvest').textContent = game.owner || 'UNASSIGNED';
  $('farmService').textContent = `${summary.available} AVAILABLE · ${summary.owned} CONTROLLED`;
  $('farmDetailText').textContent =
    `${territory.description || territory.name}. Game value: R${Math.round(game.value).toLocaleString()} · Income: R${Math.round(game.income).toLocaleString()} · Development level ${game.development}. ${territoryFarms.length} farms linked to this territory.`;
  $('aiText').textContent =
    `${levelLabel} TERRITORY · Owner: ${game.owner || 'Unassigned'} · Control: ${game.control}% · Influence: ${game.influence}% · ${summary.available} territories available in this hierarchy · ${territoryFarms.filter(f => f.status !== 'Customer').length} active farm opportunities.`;
  ...
  $('mapStatus').textContent = `Game territory selected · ${levelLabel} · ${territory.name} · Owner: ${game.owner || 'Unassigned'} · Control: ${game.control}%`;
}
