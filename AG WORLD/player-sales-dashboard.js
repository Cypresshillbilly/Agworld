...
(()=>{
 ...
 function workspaceMarkup(){
   const d=getModel();
   const saved=window.AGWorldProgression?.getState?.(),own=saved?.userId===owner?saved.missionState||{}:{},meeting=own['c3-meeting']?.meeting,outcome=own['c3-follow-up'];
   const meetingCard=meeting?'<section class="agmp-card"><h2>Your drone meeting</h2><span class="agmp-status">'+esc(outcome?.outcome||'Planned')+'</span><h3>'+esc(meeting.name)+'</h3><p>'+esc(meeting.when)+' · '+esc(meeting.model)+'</p><p>'+esc(meeting.purpose)+'</p>'+(outcome?.notes?'<p>'+esc(outcome.notes)+'</p><p>Next step: '+esc(outcome.next)+'</p>':'<p>Complete your follow-up mission after the meeting to record the outcome.</p>')+'</section>':'';
   return '<section class="agmp-card agmp-dark"><h2>Your sales activity</h2><p>Farms you have worked on, matched to their current Company relationship. These are your recorded actions, not assigned leads or quoted revenue.</p>'+cardMarkup()+'</section>'+meetingCard+'<section class="agmp-card"><h2>Your relationships</h2>'+(d.status==='loading'?'<p>Loading your recorded activity…</p>':d.status==='error'?'<p>Activity is unavailable. Try again.</p><button type="button" class="agmp-button" data-sales-retry>RETRY</button>':d.rows.length?d.rows.map(r=>'<article class="agmp-entry"><span class="agmp-status">'+esc(r.control==='company'?'Company client':r.control==='competitor'?'Competitor relationship':r.control==='neutral'?'Prospect':'Awaiting map sync')+'</span><h3>'+esc(r.name)+'</h3><p>'+r.actions+' recorded action'+(r.actions===1?'':'s')+'</p>'+(r.farm?'<button type="button" class="agmp-button agmp-secondary" data-panel-action="entity" data-value="farm:'+esc(r.id)+'">VIEW ON MAP</button>':'')+'</article>').join(''):'<p>Your sales journey starts here. Explore the territory and record your first farm interaction to build your funnel.</p>')+'</section>';
 }
