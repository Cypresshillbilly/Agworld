...
Deno.serve(async req => {
 ...
 try{
  ...
  const context={level:Number(body.context?.level)||null,xp:Number(body.context?.xp)||null,selected:String(body.context?.selected||'').slice(0,100),currentMission:String(body.context?.currentMission||'').slice(0,300)};
