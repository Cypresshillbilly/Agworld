...
(()=>{
 ...
 let wander=0,micGeneration=0,pendingMic=false;
 ...
 function checkTour(){if(!visible||tour<0||advance)return;highlight();if(steps[tour][3]()&&!['playing','loading','paused'].includes(root.dataset.playback)){advance=true;setTimeout(()=>{advance=false;if(tour>=0)step(tour+1);},400);}}
 ...
 function show(id='system-administrator',{speak=true,compact=false,walk=true}={}){ensure();if(!names[id])return;sequence++;stopListening();endTour();guide.stopBriefing();const changed=id!==active||!avatar;active=id;visible=true;wander=0;history=[];modelContext='';q('.ag-companion-send').disabled=false;
  ...
  tell(names[id],id==='system-administrator'?welcome():intros[id],speak);toggleDialogue(!compact);status('');pose();
 ...
 async function speakBriefing(text,token){
  ...
  }catch(error){voiceCache.delete(key);if(window.AGWorldVoiceBridge.isCurrent(token)){window.AGWorldVoiceBridge.error(token);status(error.message||'CHARACTER VOICE UNAVAILABLE — CLICK SPEAK TO RETRY');}}
 }
 ...
 async function ask(question){ensure();question=String(question||'').trim().slice(0,500);if(question.length<2)return;endTour();guide.stopBriefing();stopListening();toggleDialogue(true);const token=++sequence,id=active;q('.ag-companion-send').disabled=true;status('SEARCHING…');q('#agCommanderQuestion').value=question;
  try{let answer,sources=[],mode='Game guide',serviceIssue='';
   ...
   try{const result=await service({action:'chat',commander:id,question,history:history.slice(-4),searchContext:modelContext,context:{level:state().level,xp:state().xp,selected:window.AGWorldTerritoryBoard?.getSelection()?.name}});if(result.answer){answer=result.answer;sources=result.sources||[];mode='AI answer · source references';}}catch(error){serviceIssue=error.message;}
   if(token!==sequence||!visible)return;history.push({role:'user',content:question},{role:'assistant',content:answer});history=history.slice(-6);tell('You asked: '+question,answer,true,sources);status(mode+(serviceIssue?' · '+serviceIssue:''));
 ...
 function ensure(){if(root)return;root=$('agWorldSystemGuide');if(!root)throw Error('Briefing surface unavailable');guide=window.AG_WORLD_GUIDE;originalHide=guide.hide;originalWelcome=guide.welcome;
  ...
  new MutationObserver(()=>{pose();checkTour();}).observe(root,{attributes:true,attributeFilter:['data-playback']});
 ...
 window.AGWorldCompanions={show,hide,ask,speakBriefing,tourActive:()=>tour>=0,getState:()=>({active,visible,tour,dialogue:!!root&&!root.classList.contains('avatar-only'),playback:root?.dataset.playback}),startTour:()=>{show('system-administrator',{speak:false});step(0);}};
 ...
 addEventListener('agworld:player-visible',()=>{if(arrived)return;arrived=true;ensure();show('system-administrator',{speak:q('.ag-guide-arrival').checked});tour=0;q('.ag-tour-progress').textContent='WELCOME · Open MENU on the left to begin';q('.ag-tour-skip').hidden=false;highlight();},{once:true});
 ...
 setInterval(()=>{if(visible&&!document.hidden&&tour<0&&root.classList.contains('avatar-only')&&innerWidth>700&&!matchMedia('(prefers-reduced-motion: reduce)').matches){wander=wander?0:85;place(true);}},18000);
