...
Run `ingest.py --inventory <private-inventory.json> --output PRIVATE_KNOWLEDGE` with Python, pypdf and openpyxl. Inventory records contain relative and absolute source paths, extension, size and modification timestamp. Original files stay in their supplied folders. The generated library contains model catalogues, cited text extracts, duplicate references and extraction status. It is ignored by Git and must remain outside the `AG WORLD` public web root. Never publish the corpus or its source inventory to GitHub Pages.
