...
function loadCssLast(){
 if(!document.getElementById('ag-live-footer-css-last')){const l=document.createElement('link');l.id='ag-live-footer-css-last';l.rel='stylesheet';l.href='live-profile-footer.css?v=20260923';document.head.appendChild(l)}
 if(!document.getElementById('ag-footer-layout-fix-css')){const f=document.createElement('link');f.id='ag-footer-layout-fix-css';f.rel='stylesheet';f.href='footer-layout-fix.css?v=20260923';document.head.appendChild(f)}
}
...
function footerMarkup(p,l){
 ...
 return `<section class="pf-section live-progress"><div class="pf-title">YOUR PROGRESS</div><div class="ag-progress-row"><div class="ag-level-shield"><b>${esc(p.level)}</b><span>LEVEL</span></div><div class="ag-progress-main"><strong>${Number(p.xp).toLocaleString()} / ${Number(p.xpToNextLevel).toLocaleString()} XP</strong><div class="ag-progress-bar"><i style="width:${progress}%"></i></div><div class="ag-progress-foot"><span>Level ${esc(p.level)}</span><span>${Math.round(progress)}%</span></div></div></div></section><section class="pf-section live-badges"><div class="pf-title">BADGES EARNED</div><div class="ag-badges">${badgesMarkup(a)}</div></section><section class="pf-section live-leaderboard"><div class="pf-title">${esc(l?.territory||p.territory)} LEADERBOARD</div><div class="ag-leader-list">${rows}</div><div class="ag-leader-link">Regional position: ${esc(p.regionalPosition??'—')}</div></section>`;
}
function skillMarkup(p){return `<section class="missions-skill-profile"><div class="pf-title">SKILL PROFILE</div><div class="ag-skill-caption">PLAYER MASTERY</div>${skillTree(p)}</section>`}
function cleanFooter(f){if(!f)return;f.querySelectorAll('.ag-reward-drone,.ag-reward-title,.ag-reward-sub,.ag-reward-bar,.ag-reward-xp').forEach(e=>e.remove());[...f.children].forEach(x=>{if(!x.matches('.live-progress,.live-badges,.live-leaderboard'))x.remove()})}
function cleanMissionDestination(m){if(!m)return;m.querySelectorAll('.missions-badges,.missions-skill-profile').forEach(e=>e.remove())}
function placeSkillProfile(){
 const m=document.querySelector('.missions');if(!m||!profileData)return;
 cleanMissionDestination(m);
 const wrap=document.createElement('div');wrap.innerHTML=skillMarkup(profileData);const node=wrap.firstElementChild;const heading=m.querySelector('h1');
 if(heading)m.insertBefore(node,heading);else m.insertBefore(node,m.firstElementChild);
}
function render(){
 const f=document.querySelector('.bottom');if(!f||!profileData)return;
 f.classList.add('ag-profile-footer');
 cleanMissionDestination(document.querySelector('.missions'));
 f.innerHTML=footerMarkup(profileData,leaderboardData||{territory:profileData.territory,leaders:[]});
 cleanFooter(f);
 placeSkillProfile();
}
