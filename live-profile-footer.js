/* AG WORLD — live profile footer. Stable footer owner with mission badge relocation and Dota-style radar skills. */
(()=>{
...
function radarSkillData(p){
 const t=p.skillTotals||window.AG_WORLD_SKILL_TOTALS||{};
 return [
  ['TECHNICAL','Technical',Number(t.technical||0)],
  ['OPERATIONAL','Operational',Number(t.operational||0)],
  ['PRODUCT','Product',Number(t.product||0)],
  ['MANAGEMENT','Management',Number(t.management||0)],
  ['PEOPLE','People',Number(t.people||0)]
 ];
}
...
function removeLegacyReward(f){if(!f)return;f.querySelectorAll('.ag-reward-drone,.ag-reward-title,.ag-reward-sub,.ag-reward-bar,.ag-reward-xp').forEach(e=>e.remove());f.querySelectorAll('.pf-section').forEach(s=>{const t=(s.querySelector('.pf-title')?.textContent||'').trim().toUpperCase();if(t.includes('NEXT LEVEL REWARD'))s.remove()});[...f.children].forEach(x=>{if(!x.matches('.live-progress,.live-badges,.live-skills,.live-leaderboard'))x.remove()})}
function placeBadges(){
 const f=document.querySelector('.bottom'),m=document.querySelector('.missions');if(!f||!m)return;
 const source=f.querySelector('.live-badges');if(!source)return;
 let badge=m.querySelector('.missions-badges');
 if(!badge){badge=source.cloneNode(true);badge.classList.remove('live-badges');badge.classList.add('missions-badges');badge.removeAttribute('style');const pseudoTarget=m.querySelector('h1');if(pseudoTarget)m.insertBefore(badge,pseudoTarget);else m.insertBefore(badge,m.firstElementChild)}
 source.style.display='none';source.setAttribute('aria-hidden','true');
}
function render(){const f=document.querySelector('.bottom');if(!f||!profileData)return;f.classList.add('ag-profile-footer');f.innerHTML=markup(profileData,leaderboardData||{territory:profileData.territory,leaders:[]});removeLegacyReward(f);placeBadges()}
...
async function init(){
 loadCssLast();if(loading)return;const f=document.querySelector('.bottom');if(!f)return;loading=true;
 try{const p=await json(`${API}/api/users/${USER_ID}/profile`);profileData=p;window.AG_WORLD_SKILL_TOTALS=p.skillTotals||window.AG_WORLD_SKILL_TOTALS||{};try{leaderboardData=await json(`${API}/api/users/${USER_ID}/leaderboard`)}catch(e){leaderboardData={territory:p.territory,leaders:[]}}render()}
 catch(e){console.warn('AG World profile API unavailable; using local profile fallback:',e);profileData=fallbackProfile;leaderboardData={territory:fallbackProfile.territory,leaders:[{id:USER_ID,name:'Nico van Rooyen',xp:fallbackProfile.xp},{id:'user-2',name:'Sarah K.',xp:6400},{id:'user-3',name:'David L.',xp:5980}]};window.AG_WORLD_SKILL_TOTALS=fallbackProfile.skillTotals;render()}
 finally{loading=false}
}
function protect(){const f=document.querySelector('.bottom');if(!f||!profileData)return;removeLegacyReward(f);placeBadges();if(!f.querySelector('.live-progress')||!f.querySelector('.live-skills')||!f.querySelector('.live-leaderboard'))render()}
