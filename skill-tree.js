...
(()=>{
  ...
  function currentTotals(){
    ...
    const hasExisting=SKILLS.some(s=>Number(existing[s.key])>0);
    const source=hasExisting?existing:derived;
    const out=zero();
    SKILLS.forEach(s=>out[s.key]=clamp(source[s.key]));
  ...
  function companyMarkup(data){
    ...
    return '<div class="company-skill-chart"><div class="company-skill-chart-head"><div><span>COMPANY CAPABILITY</span><b>COMBINED USER SKILLS</b></div><div class="company-skill-total"><strong>'+total+' ★</strong><small>'+data.users+' ACTIVE USER'+(data.users===1?'':'S')+'</small></div></div><div class="company-skill-chart-main"><div class="company-skill-visual">'+radarSvg(totals,{id:'company',compact:true,company:true})+'</div><div class="company-skill-breakdown">'+SKILLS.map(s=>'<div class="'+(s.key===strongest.key?'is-strongest':'')+'"><span>'+s.icon+' '+s.name+'</span><b>'+totals[s.key]+'</b></div>').join('')+'</div></div><div class="company-skill-insight"><span>STRONGEST CAPABILITY</span><b>'+strongest.name+'</b></div></div>';
  }
