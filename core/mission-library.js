...
(function(){
  ...
  function inject(){
    ...
    style.textContent = `
      ...
      .gc-ml-tree{padding-top:13px}.gc-folder{border:1px solid #e3e8e3;border-radius:9px;margin-bottom:8px;overflow:hidden;background:#fbfcfb}.gc-folder-root{background:#f7f9f7}.gc-folder-head{display:flex;align-items:center;gap:9px;padding:11px 12px;font-size:10px;font-weight:900;color:#303a34}.gc-folder-head .folder-icon{font-size:14px}.gc-folder-path{margin-left:auto;font-size:7px;color:#909991;font-weight:700;letter-spacing:.7px}.gc-folder-children{padding:2px 12px 10px 34px}.gc-mission-file{display:flex;align-items:center;gap:9px;padding:9px 8px;border-top:1px solid #edf0ed;cursor:pointer}.gc-mission-file:hover{background:#f4f7f2}.gc-file-icon{font-size:12px;color:#91b322}.gc-file-name{font-size:9px;font-weight:800;color:#26302a}.gc-file-meta{margin-left:auto;font-size:7px;color:#8b958e;max-width:36%;text-align:right;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.gc-delete-mission{margin-left:8px;border:1px solid #e2caca;background:#fff7f7;color:#a33b3b;border-radius:5px;padding:5px 7px;font-size:7px;font-weight:900;letter-spacing:.6px;cursor:pointer}.gc-delete-mission:hover{background:#fbeaea}.gc-empty{padding:17px 8px;color:#87918a;font-size:9px}.gc-ml-hint{margin-top:10px;font-size:8px;color:#8a948d}.gc-ml-selected{outline:2px solid rgba(168,213,31,.45);outline-offset:-2px}
      @media(max-width:700px){.gc-ml-head{flex-direction:column}.gc-folder-path,.gc-file-meta{display:none}.gc-delete-mission{margin-left:auto}}
    `;
  ...
  function render(){
    ...
    Object.keys(groups).sort().forEach(build => {
      ...
      Object.keys(groups[build]).sort().forEach(folder => {
        ...
        groups[build][folder].forEach(mission => {
          html += `<div class="gc-mission-file" data-mission-id="${esc(mission.id)}" title="Open mission in Mission Engine"><span class="gc-file-icon">⚡</span><span class="gc-file-name">${esc(mission.name)}</span><span class="gc-file-meta">${esc(roleLabel(mission.role))}</span><button type="button" class="gc-delete-mission" data-delete-id="${esc(mission.id)}" title="Delete mission">DELETE</button></div>`;
        });
        html += `</div></div>`;
      });
      html += `</div></div>`;
    ...
    tree.querySelectorAll('.gc-delete-mission').forEach(button => button.addEventListener('click', event => {
      ...
      if (!mission) return;
      const confirmed = window.confirm(`Delete mission "${mission.name || 'Unnamed mission'}"?\n\nThis will remove it from the Mission Library.`);
      if (!confirmed) return;
