...
function seedDemoFarms() {
  ...
  // Test mode intentionally replaces the old seeded farms so only the
  // 100 green demo farms participate in the map and territory calculations.
  const existing = [];
  farms.filter(f => !f.demo && !String(f.id).startsWith('demo-farm-')).forEach(f => {
...
async function loadSpatialLayersInBackground() {
  console.info('AG World GIS loader v16 started');
